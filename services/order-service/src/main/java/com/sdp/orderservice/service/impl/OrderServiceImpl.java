...
//        // 6. Convert to DTO and publish order created event
//        OrderDTO orderDTO = mapToOrderDTO(savedOrder);
//
//        // Construct event with payment information
//        OrderCreatedEvent event = OrderCreatedEvent.builder()
//                .orderId(savedOrder.getId())
//                .customerId(savedOrder.getCustomerId())
//                .totalAmount(savedOrder.getTotalAmount())
//                .paymentMethod(orderRequest.getPaymentMethod())
//                .isOnline(orderRequest.getPaymentMethod() != PaymentMethod.CASH)
//                .returnUrl(orderRequest.getReturnUrl()) // Include return URL for payment redirect
//                .orderDTO(orderDTO)
...
//        log.info("Order created event published successfully for order ID: {}", savedOrder.getId());
//
//        // 7. Return response immediately (without waiting for payment)
...
//                .build();
//
//        // For non-cash payments, include message about payment link
//        if (orderRequest.getPaymentMethod() != PaymentMethod.CASH) {
//            response.setMessage("Order placed successfully. Payment link will be sent shortly.");
//        }
//
//        return response;

         //6. Handle payment if it's an online payment
        ...
        if (orderRequest.getPaymentMethod() != PaymentMethod.CASH) {
            ...
            try {
                log.info("Initiating payment for order in try block: {}", savedOrder.getId());
                paymentResponse = paymentServiceClient.createPayment(paymentRequest);
                log.info("Payment initiated: {}", paymentResponse);
            } catch (Exception e) {
        ...
        orderEventProducer.publishOrderCreatedEvent(orderDTO);
         ...
         //8. Return response with payment details if applicable
        OrderResponse response = OrderResponse.builder()
                .orderId(savedOrder.getId())
                .customerId(savedOrder.getCustomerId())
                .customerName(savedOrder.getCustomerName())
